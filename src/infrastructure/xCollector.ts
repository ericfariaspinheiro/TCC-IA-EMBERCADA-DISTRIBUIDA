...
// 🔹 cache simples (continua válido — é infraestrutura, não lógica de domínio)
let cache: {
  username: string
  data: { tweet: Tweet | null, replies: Tweet[] }
} | null = null

export async function getLastTweetWithReplies(username: string): Promise<{
  tweet: Tweet | null
  replies: Tweet[]
}> {
  ...
  try {
    ...
    // 🔹 2. Iterar até encontrar um tweet com replies
    for (const t of tweets) {
      ...
      // 🔹 controle de rate limit (infra, não lógica de domínio)
      await sleep(3000)
      ...
      // 🔹 4. Retornar o primeiro conjunto válido
      if (replies.length > 0) {
